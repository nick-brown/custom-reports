...
class GoogleRequest {
    ...
    protected function accountObjectMapper($xml_string)
    {
        ...
        foreach ($xml->entry as $entry) {
            ...
            foreach ($entry->children('http://schemas.google.com/analytics/2009')->property as $property) {
                $properties[str_replace('ga:', '', $property->attributes()->name)] = strval(
    ...
    protected function reportObjectMapper($xml_string)
    {
        ...
        foreach ($google_results->dataSource->property as $property_attributes) {
            $report_root_parameters[str_replace('ga:', '', $property_attributes->attributes()->name)] = strval(
                $property_attributes->attributes()->value
            );
        }
        ...
        foreach ($google_results->aggregates->metric as $aggregate_metric) {
            ...
            if (preg_match('/^(\d+\.\d+)|(\d+E\d+)|(\d+.\d+E\d+)$/', $metric_value)) {
                $report_aggregate_metrics[str_replace('ga:', '', $aggregate_metric->attributes()->name)] = floatval(
                    $metric_value
                );
            } else {
                $report_aggregate_metrics[str_replace('ga:', '', $aggregate_metric->attributes()->name)] = intval(
                    $metric_value
                );
            }
        }
        ...
        foreach ($xml->entry as $entry) {
            ...
            foreach ($entry->children('http://schemas.google.com/analytics/2009')->dimension as $dimension) {
                $dimensions[str_replace('ga:', '', $dimension->attributes()->name)] = strval(
...
class GoogleRequestReportEntry {
    ...
    /**
     * Adds key/value pairs to the dimensions array
     *
     * @param $array
     * @throws Exception if give array key or value is not a string
     */
    public function pushToDimensions($array)
    {
        if (gettype($array) === 'array') {
            foreach ($array as $k => $v) {
                if (gettype($k) === 'string' && gettype($v) === 'string') {
                    ...
                    throw new Exception('Array keys and values must be strings');
                }
            }
        }
    }
